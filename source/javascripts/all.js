...
var Filters = (function(){
  var Filters = function(element){
    ...
    this.prop = ko.observable(1)

    this.color = ko.observableArray();
    this.text_case = ko.observableArray();
    this.importance = ko.observableArray();
  ...
  $.extend(Filters.prototype,{
    add_prop: function(self, event){
      ...
      var target = $(event.target), // input[radio]
          ...
          val = target.val(),
          collection = this.color, //self[filters_group.data("filter-name")],
          index = collection.indexOf(val),
          filter_type = "color" //filters_group.data("filter-type") || "or";
      // if(filter_type == "and"){
